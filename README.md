...
⚠️ The server binds to `127.0.0.1` (localhost) only, by design — never expose it to the network. It holds your CardTrader token and reads/writes your local collection file.

### Navigating the app
The interface has 4 pages, navigable from the top bar:

#### 1. 📋 `Collezione & Valutazione` (Collection & Valuation)
The main dashboard.
- **Card Table**: Lists all your saved cards, showing passcode ID, Name, Set Code, Rarity, Quantity, NM Price, and estimates for all lower conditions (EX, GD, LP, PO), plus the grade and the effective real-world value when a card has been graded. Click a column header to sort.
...
#### 2. ➕ `Aggiungi Carta` (Add Card)
- **Search**: Italian name (`Mago Nero`), English name (`Dark Magician`), a set code (`RA01-EN001`), or a numeric passcode (`46986414`).
- Every matching card is shown with **all of its printings/sets already listed underneath** — pick the exact one you own and set the quantity, then click "Aggiungi". The real CardTrader price is looked up only at that point (no price is shown before you confirm).

#### 3. 🚀 `Aggiunta Bulk` (Bulk Add)
For adding many cards at once (e.g. after a big purchase/trade).
- Paste multiple set codes separated by spaces or newlines (e.g. `RA01-EN001 LOB-001 SDMM-IT014`) and click "Carica Codici".
- For each card in the queue, pick the correct printing and confirm, or skip it — one at a time.
- Once the queue is done, click "Salva Tutto in Collezione" to persist everything to disk.

#### 4. 🩺 `Grading Carta (CV + AI)`
- Upload a photo of the physical card and click "Analizza Carta".
...
- To save the result, search for the matching card below, pick the set/rarity, and click "Salva con Grade": a new entry is created in the collection with the grade applied, so you can compare its real value against the theoretical NM estimate in the Collection page.
